...
function setOptimizedInterval(callback, delay) {
    ...
    function loop(timestamp) {
        ...
        activeIntervals.set(id, requestAnimationFrame(loop));
    }
...
function clearOptimizedInterval(id) {
    if (activeIntervals.has(id)) {
        cancelAnimationFrame(activeIntervals.get(id));
...
function longBlink(elemID){
    ...
    { 
    var handle = setOptimizedInterval(function(){longToggleVisibility(elemID)}, 32);    
    }
...
function blink(elemID){
    ...
    { 
    var handle = setOptimizedInterval(function(){toggleVisibility(elemID)}, 30);    
    }
